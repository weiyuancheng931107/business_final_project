...
def _fetch_finmind_price_history(ticker_symbol: str, period: str) -> pd.DataFrame:
    ...
    rows = _fetch_finmind_dataset("TaiwanStockPrice", stock_id, start_date, end_date)
    if not rows:
        return pd.DataFrame()
...
def get_price_history(ticker_symbol: str, period: str = "1y") -> pd.DataFrame:
    ...
    print(f"[INFO] yfinance returned empty for {ticker_symbol}; trying FinMind.")
    return _fetch_finmind_price_history(ticker_symbol, period)
...
def get_stock_news(symbol: str, start_date_str: str = None) -> dict:
    """
    Fetch real financial news (used by Flask app).
    """
    if start_date_str:
        try:
...
def get_stock_news_range(
    ...
    *start_date*, collecting every article it finds.
    ...
    request_delay   : seconds to sleep between FinMind requests.
                      FinMind free tier = 300 req/hr → 12 s is the hard
                      minimum; 0.5 s works in practice.
                      With a registered token = 600 req/hr → 0.12 s minimum.
                      Default 0.5 s is conservative but practical.
    skip_dates      : set of "YYYY-MM-DD" strings to skip (for resume).
                      Dates in this set are not queried.
    ...
    API request budget
    ------------------
    One request per calendar day per stock symbol.
    365-day range × 10 stocks = 3,650 requests.
    At 0.5 s delay: ~30 min of FinMind queries per run.
    At 0.12 s delay (token): ~7 min.
    """
    stock_id = symbol.split(".")[0]
